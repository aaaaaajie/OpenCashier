...
import { QuickstartStore, type QuickstartOrder } from "./store";

const config = loadConfig();
const store = new QuickstartStore();
...
function renderHomePage(): string {
  const recentOrders = store.list().slice(0, 5);
    ...
    `
      ...
      <section class="grid">
        <article class="card stack">
          ...
          <p class="hint">
            Defaults assume local demo data is enabled and OpenCashier is reachable on
            <code>127.0.0.1</code>. The default single-channel path does not require the
            platform web cashier process.
          </p>
        </article>
