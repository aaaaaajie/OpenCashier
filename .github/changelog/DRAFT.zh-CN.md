...
### 新增
- 为 `@opencashier/sdk` 补齐了直接发布到 npm 所需的基础设施，包括可发布的包元数据、包内 README 与 LICENSE、打包后消费者 smoke test，以及一条使用 npm OIDC trusted publishing 的手动 GitHub 发布 workflow，不再依赖长期写权限 token。
